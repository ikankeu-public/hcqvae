...
from ag_news.common import DEVICE_IDS, run_evaluation, TOPIC_COUNT
...
def execute(director_path,train_dataset_loader):
    trainer = VAETrainer(
        model=HybridVAELLS(train_dataset_loader.get_vocab()),
        dataset_loader=train_dataset_loader,
        epoch=20,
        model_path=f"{director_path}/../hybrid-vae-(prob)-{director_path.split('/')[-1]}-{{test_acc}}#.pth",
        log_path=f"{director_path}/hybrid-vae-(prob).txt",
