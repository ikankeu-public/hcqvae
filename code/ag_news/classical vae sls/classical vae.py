...
def execute(director_path,train_dataset_loader):
    trainer = VAETrainer(
        model=ClassicalVAESLS(train_dataset_loader.get_vocab()),
        dataset_loader=train_dataset_loader,
        epoch=20,
        model_path=f"{director_path}/../classical-vae-{director_path.split('/')[-1]}-{{test_acc}}#.pth",
        log_path=f"{director_path}/classical-vae.txt",
